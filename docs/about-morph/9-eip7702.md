...
Morph supports EIP-7702, which allows Externally Owned Accounts (EOAs) to execute smart contract code within a single transaction. This enables batched operations, gas delegation, and scoped permissions without requiring migration to smart contract wallets. The implementation provides account abstraction capabilities while preserving the security and simplicity of traditional EOAs, improving flexibility in transaction workflows and multi-step operations.

## Motivation

EOAs remain the default account type for most users, yet they lack programmable features that modern applications often expect. While smart contract wallets provide richer functionality, they require deployment costs, complex onboarding, and ecosystem support that is not always available. This proposal focuses on enhancing EOAs directly, allowing common UX improvements to spread across applications without forcing users into new account formats.
...
Delegated code has unlimited authority over the account. Users must treat authorizations as seriously as handing over their private key. Wallets should not allow arbitrary delegation without proper warnings. Applications should avoid workflows where users are pressured to sign unclear or unaudited authorization entries.

## Backwards Compatibility
Morph’s implementation of EIP-7702 is fully compatible with existing EOAs. Accounts without a delegation marker behave as they do today, and existing contracts interact with these EOAs without changes. Tools using EXTCODEHASH will continue to detect the marker instead of delegated code, preserving existing assumptions about code identity.
