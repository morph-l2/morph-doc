...
|---------|---------------|-----------|
| **Alt Fee Transaction support** | ❌ Manual serialize/deserialize needed | ✅ Built-in: add `feeTokenID` (required for token gas); `feeLimit` is optional |
...
**Key fields:**
- `feeTokenID`: Token ID in the Token Registry (required when paying gas in that token)
- `feeLimit` (optional): Max token amount the user authorizes for gas; if omitted, the chain may use the sender’s token balance as the effective cap (see [AltFeeTx spec](/about-morph/10-altfeetx))
...
| `feeTokenID` | `number` | No* | Token ID (required for Alt Fee / type `0x7f`) |
| `feeLimit` | `bigint \| Hex` | No* | Optional token fee cap. If set, must be ≥ 0. Accepts bigint or hex string. |
...
const populatedTx = await signer.populateTransaction({
  ...
  value: parseEther("0.001"),
  feeTokenID: 3,
  feeLimit: 1000000n, // optional: explicit cap in token units
});
...
const txResponse = await signer.sendTransaction({
  ...
  maxPriorityFeePerGas: utils.hexlify(14116544),
  feeTokenID: 3,
  feeLimit: utils.parseUnits("1", 6), // optional: e.g. 1 USDT/USDC cap
});
```
