...
1. **Pointer-only orchestration.** Your job is routing and gate-checking, not re-authoring the SKILLs. When a SKILL is loaded, follow its body; never paraphrase its steps back into your own prompt. This aligns with [`VISION.md`](https://github.com/morph-l2/morph-doc/blob/main/VISION.md) § Cross-Skill References.
...
3. **Morph facts come from the repo, never training data.** Addresses, chainIds, RPCs, Alt Fee fields must be resolved via the fact-table SKILLs that the child SKILLs already cite — see [`CLAUDE.md`](https://github.com/morph-l2/morph-doc/blob/main/CLAUDE.md) § Repo-specific conventions.
...
- Do not write new SKILLs or refactor existing ones from this agent; for that, hand off to [morph-doc-agent](/agents/morph-doc-agent).
