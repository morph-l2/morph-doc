/**
 * MorphNetwork RPC API - HTTP Mode Configuration
 */
...
function getDefaultValue(schema: any): string {
  ...
  if (schema.$ref) {
    ...
    if (refName === 'BlockHash' || refName === 'TransactionHash') {
...
export const rpcHttpConfig: ApiExplorerConfig = {
  title: 'Morph JSON-RPC API',
  description: 'Standard Ethereum JSON-RPC API, supports HTTP calls.',
  mode: 'http',
