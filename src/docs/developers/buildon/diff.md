...
It's important to note that there are various minor discrepancies between the behavior of Morphism and Ethereum.
You should be aware of these descrepancies when building apps on top of Morphism.



## Blocks

There are several differences in the way blocks are produced between L1 Ethereum and Morphism Bedrock.


| Parameter           | L1 Ethereum | Morphism |
| - | - | - |
| Time between blocks | 12 seconds(1)  | 1 second |
| Block Generation   | fix timeslot | fix timeslot|
| Empty blocks?  | no empty block but could be missed | 5 seconds if no txns | 

(1) This is the ideal. 
    If any blocks are missed it could be an integer multiple such as 24 seconds, 36 seconds, etc.

**Note:** The L1 Ethereum parameter values are taken from [ethereum.org](https://ethereum.org/en/developers/docs/blocks/#block-time). <!-- The Morphism values are taken from [the Morphism specs](https://github.com/ethereum-optimism/optimism/blob/develop/specs/guaranteed-gas-market.md#limiting-guaranteed-gas). -->

If you want to use timestamp as usage on token vesting etc, just use block.timestamp like what you did on Ethereum.
...
However, on Morphism `tx.origin` is the origin *on Morphism*.
It could be an EOA.
...
If you need the equivalent information from the latest L1 block, you can get it from [the `L1Block` contract]().

This contract is a predeploy at address [`0x4200000000000000000000000000000000000015`]().
...
1. Before Nimrod's swap transaction is sent to the blockchain, Helena Hacker sends a transaction from an L1 contract with the same address as Hackswap.
   ...
   Nimrod, despite his naivete, is protected because Morphism modified the transaction's `tx.origin` (which is also the initial `msg.sender`).
...
Morphism uses the same [JSON-RPC API](https://eth.wiki/json-rpc/API) as Ethereum.
Some additional Morphism specific methods have been introduced.
See the full list of [custom JSON-RPC methods](./json-rpc.md) for more information.

## Transaction costs

[Transaction costs on Morphism](./txfee.md) include an [L2 execution fee](./txfee.md#the-l2-execution-fee) and an [L1 data fee](./txfee.md#the-l1-data-fee). 


## Contract addresses

The addresses in which various infrastructure contracts are installed are different between L1 Ethereum and Morphism.
...
On Morphism the same contract is installed on [address `0x4200000000000000000000000000000000000006`]().
